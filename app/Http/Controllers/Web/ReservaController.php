...
class ReservaController extends Controller
{
    ...
    {
        $reservas = Reserva::with(['usuario', 'detalleReservas.habitacion', 'servicios', 'factura'])->orderBy('creado_en', 'desc')->get();
    ...
    public function store(Request $request)
    {
        try {
            ...
            return DB::transaction(function () use ($validatedData) {
                ...
                return redirect()->route('reservas.index')->with('success', 'Registro creado exitosamente.');
            });
        } catch (Exception $ex) {
    ...
    public function update(Request $request, string $id)
    {
        try {
            ...
            if (in_array($reserva->estado, ['confirmada', 'completada'], true) && ($reserva->fecha_entrada->format('Y-m-d') !== $validatedData['fecha_entrada'] || $reserva->fecha_salida->format('Y-m-d') !== $validatedData['fecha_salida'])) {
                return redirect()->back()->withInput()->with('error', 'No se pueden modificar fechas de una reserva confirmada o completada.');
            }
    ...
    public function destroy(string $id)
    {
        try {
            ...
            if (in_array($reserva->estado, ['confirmada', 'completada'], true)) {
                return redirect()->back()->with('error', 'No se puede eliminar una reserva confirmada o completada.');
            }
